...
- **管理后台** `/admin`：登录后可
  - 查看所有上传记录（文件名/类型/大小/时间），**下载源文件（原格式，服务器代理拉取七牛文件流，中文文件名正常）**、删除记录
...
- **上传记录**：访客每次上传的原文件都会出现在这里（记录含七牛 URL），点「下载」由服务器**代理拉取七牛文件流**下载**原格式文件**（中文文件名不乱码）；可删除记录
...
- **注意**：七牛测试域名（`*.hn-bkt.clouddn.com`）仅支持 `http://` 访问，系统生成的文件 URL 默认使用 `http://`；正式绑定支持 HTTPS 的自定义域名后，可在 `utils/qiniu.js` 中把 URL 改回 `https://`（管理后台下载走服务器代理，不受影响）
